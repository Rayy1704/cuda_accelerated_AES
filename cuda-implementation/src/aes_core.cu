...
__device__ void add_round_key(aes_state*state,unsigned char *expanded_keys,int round){
...
__host__ __device__ unsigned char xtime(unsigned char x) {
    return (x << 1) ^ (((x >> 7) & 1) * 0x1b);
}
__host__ __device__ unsigned char multiply(unsigned char a, unsigned char b) {
    if (a == 1) return b;
...
__device__ void populate_state(aes_state * state, unsigned char * input){
