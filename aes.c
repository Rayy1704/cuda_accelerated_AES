...
void shift_rows(aes_state*state){
    ...
    for (int c = 0; c < 3; c++) {
        state->matrix[1][c] = state->matrix[1][c + 1];
    }
    state->matrix[1][3] = temp;
    // Row 2: shift left by 2
    unsigned char temp1 = state->matrix[2][0];
    unsigned char temp2 = state->matrix[2][1];
    for (int c = 0; c < 2; c++) {
        state->matrix[2][c] = state->matrix[2][c + 2];
    }
    state->matrix[2][2] = temp1;
    state->matrix[2][3] = temp2;
    // Row 3: shift left by 3 (or right by 1)
    temp = state->matrix[3][3];
    for (int c = 3; c > 0; c--) {
        state->matrix[3][c] = state->matrix[3][c - 1];
    }
